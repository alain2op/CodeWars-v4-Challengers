...
def moveTo(x, y, pirate):
    x=int(x)
    y=int(y)
    position = (int(pirate.getPosition()[0]),int(pirate.getPosition()[1]))
...
def vertexMove(bx,by,pirate):
    id=int(pirate.getID())
    paths=int(95*pirate.getDimensionX()/40)
    ...
    if id % paths == 0:
        ...
        if id % 2 == 0:
            ...
            lx,ly=dx-1-bx,by
            mx,my=int(((n-1)*kx+(p-n+1)*lx)/p),int(((n-1)*ky+(p-n+1)*ly)/p)
            #mx,my=int(((p-n)*kx+n*lx)/p),int(((p-n)*ky+n*ly)/p)
            string=str(mx).zfill(2)+str(my).zfill(2)+pirate.getSignal()[4:100]
            pirate.setSignal(string)
            return moveTo(mx, my, pirate)
        else:
            ...
            if (px==tx) and (py==ty):
                if( px==0 or px==dx-1 ) and (py==0 or py==dy-1):
                    ...
                    if (px==0 or px==dx-1):
                        ...
                        string=str(mx).zfill(2)+str(my).zfill(2)
                        pirate.setSignal(string)
                        return moveTo(mx,my,pirate)
    ...
    #             return moveTo(targetx,targety,pirate)
        ...
        if (1 in dirs1 and checkIsland(pirate)!=(0,0)):
            ...
            if(pirate.getTeamSignal()[start:start+4])=="0000":
                ...
                s=s[:start]+isignal+s[start+4:]
                pirate.setTeamSignal(s)
        # work=bool(pirate.getSignal()[4:7]!="000")
        # island=100
        # if(work):
        ...
        #     pirate.setSignal(string)
        if(pirate.getCurrentFrame()>2000):
            if("oppCaptured"in track):
                i=track.index("oppCaptured")-3
                x,y=pirate.getTeamID()[i*4:i*4+2],pirate.getTeamID()[i*4+2:i*4+4]
                if(id%10==0):
                    return moveTo(x,y,pirate)
                else:
                    return circleAround(x,y,1,pirate)
        if(pirate.getCurrentFrame()>2500):
            i=int(pirate.getID())%3
            x,y=int(pirate.getTeamSignal()[i*4:i*4+2]),int(pirate.getTeamSignal()[i*4+2:i*4+4])
            if(id%10==0):
                return moveTo(x,y,pirate)
            else:
                return circleAround(x,y,1,pirate)
        if(id%5<3):
            if(track[id%5+3]=="oppCapturing" and pirate.getTeamSignal()[(id%5)*4:(id%5)*4+4]!="0000" and pirate.getCurrentFrame()>250):
                x,y=pirate.getTeamSignal()[(id%5)*4:(id%5)*4+2],pirate.getTeamSignal()[(id%5)*4+2:(id%5)*4+4]
                return moveTo(x,y,pirate)
...
def ActTeam(team):
    ...
    if(team.getTeamSignal()==""):
        ...
        n=int(team.getTotalPirates())
        string=team.getTeamSignal()[:50]+str(n).zfill(3)+team.getTeamSignal()[53:]
        team.setTeamSignal(string)
